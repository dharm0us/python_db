...
def initialize_leaf_node(node):
    ...
    # 0 represents no sibling (sibling page numbering starts at 0, but 0 is root, so it's safe)
    # Actually, 0 IS a valid page (root), so 0 is ambiguous if next_leaf points to root?
    # In this DB, leaves are never root if there are multiple levels. 
    # But strictly, we treat 0 as "No Next Page" logic inside cursor_advance.
    struct.pack_into('<I', node, LEAF_NODE_NEXT_LEAF_OFFSET, 0)
...
def set_leaf_node_next_leaf(node, next_leaf_page):
    struct.pack_into('<I', node, LEAF_NODE_NEXT_LEAF_OFFSET, next_leaf_page)
...
def leaf_node_split_and_insert(cursor, key, value):
    ...
    for i in range(LEAF_NODE_MAX_CELLS, -1, -1):
        ...
        if i == cursor.cell_num:
            # Replaced "serialize_row at destination" with new logic
            # to match strict C-diff behavior (explicit key set + value serialize)
            # Actually, serialize_row helper still writes bytes, but key is separate now
...
def leaf_node_insert(cursor, key, value):
    ...
    # The diff says: serialize_row(value, leaf_node_value(...))
    # We calculate offset manually to match serialize_row signature
...
def internal_node_find(table, page_num, key):
    ...
    elif child_type == NodeType.INTERNAL:
...
def table_start(table):
    # NEW LOGIC: Find key 0 (min key) to locate left-most leaf
...
def execute_insert(statement, table):
    ...
    num_cells = leaf_node_num_cells(node) # Only for early full check if single node? No, split handles it.
...
def cursor_advance(cursor):
    ...
    if cursor.cell_num >= leaf_node_num_cells(node):
        # NEW LOGIC: Advance to next leaf node
